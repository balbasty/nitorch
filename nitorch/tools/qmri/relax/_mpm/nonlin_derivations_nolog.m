...
R1 = lR1; % exp(lR1);
R2 = lR2; % exp(lR2);
d  = ld; % 1 / (1 + exp(-ld));        % delta = MT ratio
md = 1 - d;                     % 1 - delta
...
gg = [S/A; ...
      -TE * S; ...
      -TR * e1 * S * (md .* cosa - 1) ./ ((1 - e1) .* (1 - md .* cosa .* e1));...
      -S / (md * (1 - md * cosa * e1))];
HH0 = gg * gg.';
gg = gg * R;
...
% HHs = [0; ...
%        ggs(2) * (-(- TE) * R2^2 - 2 * R2); ...
%        ggs(3) * (-(- TR * l_r1) * R1^2 - 2 * R1); ...
%        ggs(4) * (- 2 * cosa * e1) / (1 - beta * e1)];
% HHs(5) = ggs(2)/A;  % PD/R2
% HHs(6) = ggs(3)/A;  % PD/R1
% HHs(7) = ggs(4)/A;  % PD/MT
% HHs(8) = ggs(3) * (-TE) * (-R2^2); % R2/R1
% HHs(9) = ggs(4) * (-TE) * (-R2^2); % R2/MT
% HHs(10) = -ggs(3) * k_mt + S * cosa * e1 * TR / (1 - cosa * md * e1)^2 * (-R1^2); % R1/MT
...
HHs(6) = ggs(3)/A;  % PD/R1
HHs(7) = ggs(4)/A;  % PD/MT
HHs(8) = ggs(3) * (-TE); % R2/R1
HHs(9) = ggs(4) * (-TE); % R2/MT
HHs(10) = -ggs(3) * k_mt + S * cosa * e1 * TR / (1 - cosa * md * e1)^2; % R1/MT
...
simplify(Hs(1,2)-HHs(5), 100)
simplify(Hs(1,3)-HHs(6), 100)
simplify(Hs(1,4)-HHs(7), 100)
simplify(Hs(2,3)-HHs(8), 100)
simplify(Hs(2,4)-HHs(9), 100)
simplify(Hs(3,4)-HHs(10), 100)
