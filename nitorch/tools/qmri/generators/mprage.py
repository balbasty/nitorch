...
def mprage(pd, r1, r2s=None, transmit=None, receive=None, gfactor=None,
           te=3e-3, tr=2.3, ti=0.9, fa=9, n=100, sigma=None, noise='rician',
           device=None):
    ...
    e1 *= fa ** (2*n - 1)
    signal += ep * (1 - ei)
    signal += ep * ei * (1 - ed + fa*ed*(1-ep) * (1 - (fa*ep)**(2*n-1)) / (1 - fa*ep) + fa*ed*((fa*ep)**(2*n-1)) - e1) / (1 + e1)
    del ei, ep
    signal *= pd
    del pd
    ...
    if sigma:
        ...
        if gfactor is not None:
            noise_sample *= gfactor
        if noise == 'rician':
            noise_sample = noise_sample.square_().sum(dim=0)
            signal = signal.square_().add_(noise_sample).sqrt_()
        else:
            signal += noise_sample
