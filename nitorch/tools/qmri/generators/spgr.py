...
def spgr(pd, r1, r2s=None, mt=None, transmit=None, receive=None, gfactor=None,
    ...
    """Simulate data generated by a Spoiled Gradient-Echo (FLASH) sequence.
    ...
    all_var += [r2s.volume] if r2s is not None and torch.is_tensor(
        r2s.volume) else []
    all_var += [mt.volume] if mt is not None and torch.is_tensor(
        mt.volume) else []
    ...
    for n in range(nb_contrasts):
        ...
        if aff1 is not None:
            ...
            f1pd, f1r1, f1r2s, f1mt = exp_maps(f1pd, f1r1, f1r2s, f1mt,
                                               inplace=inplace)
        else:
