                   ...
                   int extrapolate)
{
  ...
  if (input.is_cuda()) {
    auto pair = cudapushpull(input, grid, grad,
      BoundVectorRef(bound_mode), InterpolationVectorRef(interpolation_mode), 
      extrapolate, /*pull*/false,
      /*push*/input.requires_grad(), /*count*/false,
      /*grad*/grid.requires_grad(), /*sgrad*/false);
      if (input.requires_grad())
        output.push_back(pair.left);
      if (grid.requires_grad())
        output.push_back(pair.right);
  } else {
    auto pair = cpu::pushpull(input, grid, grad,
      BoundVectorRef(bound_mode), InterpolationVectorRef(interpolation_mode), 
      extrapolate, /*pull*/false,
      /*push*/input.requires_grad(), /*count*/false,
      /*grad*/grid.requires_grad(), /*sgrad*/false);
                   ...
                   int extrapolate)
{
  std::deque<Tensor> output;
  if (input.is_cuda()) {
    auto pair = cudapushpull(grad, grid, input,
      BoundVectorRef(bound_mode), InterpolationVectorRef(interpolation_mode), 
    ...
    auto pair = cpu::pushpull(grad, grid, input,
      BoundVectorRef(bound_mode), InterpolationVectorRef(interpolation_mode), 
      extrapolate, /*pull*/input.requires_grad(), /*push*/false, /*count*/false,
      /*grad*/grid.requires_grad(), /*sgrad*/false);
    if (input.requires_grad())
                    ...
                    int extrapolate)
{
  if (grid.is_cuda()) {
    return cudapushpull(grad, grid,
      ...
      /*grad*/ grid.requires_grad(), /*sgrad*/ false).right;
  } else {
